...
if ! tmux set-option -t "=$SESSION" @repo "$REPO" || \
   ! tmux set-option -t "=$SESSION" @engine "$ENGINE" || \
   ! tmux set-option -t "=$SESSION" @toliki_kind "$KIND" || \
   { [[ -n "$MODE" ]] || tmux set-option -t "=$SESSION" @worktree "$MANUAL_WORKTREE"; } || \
   { [[ -n "$MODE" ]] || tmux set-option -t "=$SESSION" @branch "$MANUAL_BRANCH"; }; then
