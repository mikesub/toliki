...
manual_load() { # repo, session; fills MANUAL_*; 0 owned, 1 absent, 2 ambiguous
  local repo="$1" session="$2" project branch worktree engine token recorded_repo top actual_branch
  ...
  if [[ ! -d "$worktree" ]] || ! top="$(git -C "$worktree" rev-parse --show-toplevel 2>/dev/null)" || \
     [[ "$top" != "$worktree" ]] || ! actual_branch="$(git -C "$worktree" symbolic-ref --quiet --short HEAD 2>/dev/null)" || \
