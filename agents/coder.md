...
2. **Tests are the spec** — never weaken, skip, or delete a test to make it pass. If a test is genuinely wrong, fix the SMALLEST thing and note why rather than silently editing it away.
3. **Stay in scope** — build what the architecture specifies. If you hit something it didn't anticipate, take the smallest reasonable choice and note it rather than expanding scope.

## Verify, don't commit

- After changes, run the project's verification command (`npm run verify` in each touched package). It is the single authoritative gate — trust its exit code; fix failures and re-run until green.
...
TDD does not apply to doc-writing tasks. Write the file precisely to the structure your task requests, then make any requested bookkeeping update (e.g. `epic.md` phase/phase-log). Don't implement or fix code as a side effect.
